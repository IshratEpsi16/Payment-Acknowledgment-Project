...
// Add a function to handle updating status
const updateStatusScblFO = (response, payeeId, currentPeriod) => {
    const authToken = localStorage.getItem("token");
    ...
    axios.put(`http://192.168.7.15:8081/updateStatusScblFO/${payeeId}/${currentPeriod}/${response.toLowerCase()}`,{},config)
        .then(res => {
    ...
    return (
        ...
        <div className='container'>
            ...
            <h1 className='mt-4 text-xl text-center font-bold text-teal-600'>Seven Circle Bangladesh Ltd - Kaligonj </h1>
...
<p className='text-sm text-gray-600'>Dear {item.EMPLOYEE_NAME},<br></br></p>
    ...
    <button type="button" className=" btn btn-success  " style={{width:'20%', color:'white',border: '2px solid yellow'}} onClick={() => updateStatusScblFO('Yes', item.PAYEE_ID, item.PAYROLL_MONTH)}>Yes</button>
                            &nbsp; &nbsp;
                            <button type="button" style={{width:'20%', color:'white',border: '2px solid yellow'}} className="btn btn-error" onClick={() => updateStatusScblFO('No', item.PAYEE_ID, item.PAYROLL_MONTH)}>No</button>
    </div>
  </div>
</div>
...
export default User_Homepage_SCBLFO;